...
namespace billy.Middleware.Server
{
    ...
    public class RecertCheck
    {
        ...
        private async Task InitEnv()
        {
            ...
            uiUsers = await apiConnectionMiddlewareServer.SendQueryAsync<List<UiUser>>(billy.Api.Client.Queries.AuthQueries.getUsers);
            owners = await apiConnectionMiddlewareServer.SendQueryAsync<List<FwoOwner>>(billy.Api.Client.Queries.OwnerQueries.getOwners);
        }
        ...
        private async Task setOwnerLastCheck(FwoOwner owner)
        {
            ...
            await apiConnectionMiddlewareServer.SendQueryAsync<object>(billy.Api.Client.Queries.OwnerQueries.setOwnerLastCheck, Variables);
        }
    }
}
