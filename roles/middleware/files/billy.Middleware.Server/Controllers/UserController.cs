...
namespace billy.Middleware.Controllers
{
    /// <summary>
    ...
    public class UserController : ControllerBase
    {
        ...
        {
            List<UiUser> users = (await apiConnection.SendQueryAsync<UiUser[]>(billy.Api.Client.Queries.AuthQueries.getUsers)).ToList();
            ...
            if(userAdded)
            {
                ...
                try
                {
                    var Variables = new
                    {
                        ...
                        uiuser_username = (new billy.Api.Data.DistName(parameters.UserDn)).UserName,
                        email = email,
                        tenant = parameters.TenantId,
                        passwordMustBeChanged = parameters.PwChangeRequired,
                        ldapConnectionId = (parameters.LdapId != 0 ? parameters.LdapId : (int?)null)
                    };
                    ReturnId[]? returnIds = (await apiConnection.SendQueryAsync<NewReturning>(billy.Api.Client.Queries.AuthQueries.addUser, Variables)).ReturnIds;
                    if(returnIds != null)
                    {
        ...
        public async Task<bool> Change([FromBody] UserEditParameters parameters)
        {
            ...
            if (userUpdated) 
            {
                ...
                try
                {
                    ...
                    await apiConnection.SendQueryAsync<ReturnId>(billy.Api.Client.Queries.AuthQueries.updateUserEmail, Variables);
                }
                catch (Exception exception)
                {
                    userUpdated = false;
        ...
        public async Task<bool> Delete([FromBody] UserDeleteParameters parameters)
        {
            ...
            if (userDeleted)
            {
                ...
                try
                {
                    ...
                    await apiConnection.SendQueryAsync<ReturnId>(billy.Api.Client.Queries.AuthQueries.deleteUser, Variables);
                }
                catch (Exception exception)
                {
                    userDeleted = false;
        ...
        private async Task<UiUser?> resolveUser(int id)
        {
            ...
            try
            {
                uiUsers = (await apiConnection.SendQueryAsync<UiUser[]>(billy.Api.Client.Queries.AuthQueries.getUsers)).ToList();
