...
namespace billy.Middleware.Server
{
    ...
    public class DailyCheckScheduler
    {
        ...
        {
            List<Management> managements = await apiConnection.SendQueryAsync<List<Management>>(billy.Api.Client.Queries.DeviceQueries.getManagementsDetails);
            ...
            List<ImportCredential> credentials = await apiConnection.SendQueryAsync<List<ImportCredential>>(billy.Api.Client.Queries.DeviceQueries.getCredentials);
            ...
            List<UiUser> users = await apiConnection.SendQueryAsync<List<UiUser>>(billy.Api.Client.Queries.AuthQueries.getUsers);
            ...
            List<Tenant> tenants = await apiConnection.SendQueryAsync<List<Tenant>>(billy.Api.Client.Queries.AuthQueries.getTenants);
        ...
        {
            List<ImportStatus> importStati = await apiConnection.SendQueryAsync<List<ImportStatus>>(billy.Api.Client.Queries.MonitorQueries.getImportStatus);
            int importIssues = 0;
            object jsonData;
