...
do
   ...
   no_of_mgmgs_unwanted=$(psql -qtAX -d billydb -c "select count(*) from management where mgm_id IN ($sql_code)")
   next_mgm_to_delete=$(psql -qtAX -d billydb -c "select mgm_id from ($sql_code) AS old_mgm_ids limit 1")
   ...
   echo "executing: time psql -qtAX -d billydb -c delete from management where mgm_id=$next_mgm_to_delete"
   #time psql -d billydb -c "delete from management where mgm_id='$next_mgm_to_delete'"
done
