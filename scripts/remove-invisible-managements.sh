...
while true
do
   no_of_mgmgs=$(psql -qtAX -d billydb -c "select count(*) from management ")
   no_of_mgmgs_unwanted=$(psql -qtAX -d billydb -c "select count(*) from management where do_not_import and hide_in_gui")
   mgm_to_delete=$(psql -qtAX -d billydb -c "select mgm_name from management left join import_control using (mgm_id) where do_not_import and hide_in_gui group by mgm_id order by count(control_id) limit 1")
   ...
   echo "executing: time psql -qtAX -d billydb -c delete from management where mgm_name=$mgm_to_delete"
   time psql -d billydb -c "delete from management where mgm_name='$mgm_to_delete'"
done
